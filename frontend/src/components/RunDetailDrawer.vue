...
function baseNodeKey(node:any){return String(node.node_key||"").replace(/\[server:\d+\]$/," ").replace(/\[\d+\]$/," ").trim()}
function topologicalRanks(){
  const definition=detail.value?.definition||{},definitions=definition.nodes||[],definitionIndex=new Map(definitions.map((node:any,index:number)=>[node.id,index]));
  const incoming=new Map(definitions.map((node:any)=>[node.id,0])),outgoing=new Map(definitions.map((node:any)=>[node.id,[]]));
  for(const edge of definition.edges||[]){if(!incoming.has(edge.to)||!outgoing.has(edge.from))continue;incoming.set(edge.to,(incoming.get(edge.to)||0)+1);outgoing.get(edge.from).push(edge.to)}
  const queue=definitions.filter((node:any)=>(incoming.get(node.id)||0)===0).sort((a:any,b:any)=>(definitionIndex.get(a.id)||0)-(definitionIndex.get(b.id)||0));
  const ranks=new Map<string,number>();let rank=0;
  while(queue.length){const node=queue.shift();if(ranks.has(node.id))continue;ranks.set(node.id,rank++);for(const target of outgoing.get(node.id)||[]){incoming.set(target,(incoming.get(target)||0)-1);if(incoming.get(target)===0){queue.push(definitions.find((item:any)=>item.id===target));queue.sort((a:any,b:any)=>(definitionIndex.get(a.id)||0)-(definitionIndex.get(b.id)||0))}}}
  for(const node of definitions)if(!ranks.has(node.id))ranks.set(node.id,rank++);
  return ranks;
}
function compareExecutionOrder(a:any,b:any){
  const aTime=a.started_at?new Date(a.started_at).getTime():Number.POSITIVE_INFINITY,bTime=b.started_at?new Date(b.started_at).getTime():Number.POSITIVE_INFINITY;
  if(aTime!==bTime)return aTime-bTime;
  const ranks=topologicalRanks(),rankDiff=(ranks.get(baseNodeKey(a))??Number.MAX_SAFE_INTEGER)-(ranks.get(baseNodeKey(b))??Number.MAX_SAFE_INTEGER);
  if(rankDiff)return rankDiff;
  const serverDiff=(a.server_index??-1)-(b.server_index??-1);return serverDiff||a.id-b.id;
}
const orderedNodes=computed(()=>[...(detail.value?.nodes||[])].sort(compareExecutionOrder));
...
async function refresh() {
  ...
  detail.value.nodes=[...(detail.value.nodes||[])].sort(compareExecutionOrder);
  if(!selected.value&&orderedNodes.value.length)selected.value=orderedNodes.value.find((node:any)=>node.status==="running")||orderedNodes.value[0];else if(selected.value){selected.value=detail.value.nodes.find((node:any)=>node.id===selected.value.id)||selected.value;const running=orderedNodes.value.find((node:any)=>node.status==="running");if(selected.value.status==="pending"&&running)selected.value=running;}
  await loadLogs();
...
const serverGroups=computed(()=>{const groups=new Map<number,any>();for(const node of orderedNodes.value){if(!node.server_connection_id)continue;if(!groups.has(node.server_connection_id))groups.set(node.server_connection_id,{id:node.server_connection_id,index:node.server_index,server:node.server,nodes:[]});groups.get(node.server_connection_id).nodes.push(node)}return[...groups.values()].sort((a,b)=>compareExecutionOrder(a.nodes[0],b.nodes[0]))});
