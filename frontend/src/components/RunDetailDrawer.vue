...
import { CheckCircle2, XCircle, Clock3, LoaderCircle, Ban, Terminal, GitCommit, Box, ServerCog } from "lucide-vue-next";
...
async function refresh() {
  ...
  if(!selected.value&&detail.value.nodes?.length)selected.value=detail.value.nodes[0];else if(selected.value)selected.value=detail.value.nodes.find((node:any)=>node.id===selected.value.id)||selected.value;
  await loadLogs();
  if(!["queued","running"].includes(detail.value.status))clearInterval(timer);
}
function selectNode(node:any){selected.value=node;loadLogs();}
...
watch(() => [props.modelValue, props.projectId, props.runId], async ([open]) => {
  ...
  selected.value=undefined;logs.value=[];await refresh();
  if (["queued","running"].includes(detail.value?.status)) timer=setInterval(refresh,1500);
}, { immediate:true });
